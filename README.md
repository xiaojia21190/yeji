...
巨潮预约披露表 1 次调用筛出回看窗口（3 天）内新披露的公司 → 按单次上限 40 份自动生成
**完整结构报告**（评分卡 + 前景与催化 + 预期反推 + 趋势 + 估值 + 同行）：数字全部脚本可复算，
定性段落由 AI 撰写并显著标注「未经搜索溯源」（AI 走仓库 Secrets，OpenAI 兼容协议，
不配置则降级纯量化）→ 提交进仓库（Pages 自动更新）→ issue 通知。
